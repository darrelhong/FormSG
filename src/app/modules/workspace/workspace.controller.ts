...
> = async (req, res) => {
  const userId = (req.session as AuthedSessionData).user._id
  const { workspaceId: sourceWorkspaceId } = req.params
  const { formIds, destWorkspaceId } = req.body

  return WorkspaceService.checkWorkspaceExists(sourceWorkspaceId)
    .andThen(() => WorkspaceService.checkWorkspaceExists(destWorkspaceId))
    .andThen(() =>
      WorkspaceService.verifyWorkspaceAdmin(sourceWorkspaceId, userId),
    )
    .andThen(() =>
    ...
    .mapErr((error) => {
      logger.error({
        message: 'Error moving forms to anotehr workspace',
        meta: {
          action: 'moveForms',
          sourceWorkspaceId,
          destWorkspaceId,
